$(document).ready(function () {
	...
	/*動態置換內頁，若不懂為何要特意獨立這寫法出來請問我@Harmony*/
	  $(document).on('click', ".nametag-link" , function(){
		  //抓出內頁ID
		  var uni_il_id = $(this).attr('id');
		  //命名邏輯為ip_xxx_id_y, xxx { fc, pro, art}, y為整數
		  //start of first if-else
		  if (uni_il_id.substring(0,2)==="il"){
			  //先判斷是否為fc_id
			  //start of second if-else
			  if(uni_il_id.substring(3,5)==="fc"){
				  var fc_id = parseInt(uni_il_id.replace( /[^\d.]/g, '' ), 10);
				  console.log("this is fc_id" + uni_il_id);
		  		  $.ajax({
				  ...
				  if(uni_il_id.substring(3,5)==="pr"){
					  var pro_id = parseInt(uni_il_id.replace( /[^\d.]/g, '' ), 10);
					  console.log("this is pro_id : " + uni_il_id);
			  		  $.ajax({
					  ...
					  if(uni_il_id.substring(3,5)==="ar"){
						  var art_id = parseInt(uni_il_id.replace( /[^\d.]/g, '' ), 10);
						  console.log("this is art_id : " + uni_il_id);
				  		  $.ajax({
						  ...
						  //三者皆非的狀況
						  console.log("!!!!!!!!!!!!!!!!!!!!")
						  console.log("無法辦別該uni_il_id字串。")  
						  console.log(uni_il_id)
